      ...
      if (tipo == "status" && dados != null) {
        setState(() {
          ...
          if (dados.containsKey("andar_atual"))
            _andarAtual = dados["andar_atual"];
          if (dados.containsKey("temperatura"))
            _temperatura = (dados["temperatura"] as num).toDouble();
          if (dados.containsKey("capacidade_maxima_kg"))
            _capacidadeMaximaKg = dados["capacidade_maxima_kg"];
          dados.containsKey("direcao_movimentacao")
              ? _direcaoMovimentacao = dados["direcao_movimentacao"]
              : _direcaoMovimentacao = null;
          dados.containsKey("mensagens")
              ? _mensagens = List<String>.from(dados["mensagens"])
              : _mensagens = null;
          if (dados.containsKey("nome_obra")) _nomeObra = dados["nome_obra"];
          if (dados.containsKey("codigo_obra"))
            _codigoObra = dados["codigo_obra"];
        });
      }
    } catch (e) {
  ...
  Widget _buildStatusWidget() {
    return Column(
      ...
      children: [
        ...
        if (_capacidadeMaximaKg != null)
          Text("Capacidade Máxima (kg): $_capacidadeMaximaKg"),
        if (_direcaoMovimentacao != null)
          Text("Direção de Movimentação: $_direcaoMovimentacao"),
